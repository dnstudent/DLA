...
class TemperatureRegressor(nn.Sequential):
    def __init__(self, n_weather_features, dropout_rate):
        super().__init__(
            nn.Linear(n_weather_features + 1, 5),
            nn.Dropout(dropout_rate),
            nn.ELU(alpha=1.0),
            nn.Linear(5, 1)
        )
...
class LitTZRegressor(L.LightningModule):
    def __init__(self, regressor, n_weather_features, initial_lr, lr_decay_rate, weight_decay, density_lambda, dropout_rate, **kwargs):
        super().__init__()
        self.density_regressor = torch.compile(regressor)
        self.temperature_regressor = torch.compile(TemperatureRegressor(n_weather_features, dropout_rate), fullgraph=True)
...
class LitLSTM(LitTZRegressor):
    def __init__(self, n_weather_features, initial_lr, lr_decay_rate, weight_decay, density_lambda=5, dropout_rate=0.2):
        regressor = LSTMDensityRegressor(n_weather_features, dropout_rate)
        super().__init__(regressor, n_weather_features, initial_lr, lr_decay_rate, weight_decay, density_lambda, dropout_rate)
...
class LitPGLLSTM(LitTZRegressor):
    def __init__(self, z_mean, z_std, t_mean, t_std, n_weather_features, initial_lr, lr_decay_rate, weight_decay, physics_penalty_lambda, ttoz_penalty_lambda, density_lambda=5, dropout_rate=0.2):
        regressor = LSTMDensityRegressor(n_weather_features, dropout_rate)
        super().__init__(regressor, n_weather_features, initial_lr, lr_decay_rate, weight_decay, density_lambda, dropout_rate)
...
class LitPGALSTM(LitTZRegressor):
    ...
    def __init__(self, n_weather_features, initial_lr, lr_decay_rate, weight_decay=0.05, hidden_size=8, forward_size=5, density_lambda=5, dropout_rate=0.2):
        regressor = PGADensityLSTM(n_weather_features, hidden_size, forward_size, dropout_rate)
        super().__init__(regressor, n_weather_features, initial_lr, lr_decay_rate, weight_decay, density_lambda, dropout_rate)
        self.save_hyperparameters("initial_lr", "lr_decay_rate", "weight_decay", "dropout_rate", "density_lambda")
