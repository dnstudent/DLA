...
# class TimeAxisPreserver(nn.Module):
#     def __init__(self, *args, **kwargs):
#         super(TimeAxisPreserver, self).__init__(*args, **kwargs)
#
#     def _forward(self, x, *args, **kwargs):
#         raise NotImplementedError
#
#     def forward(self, x, *args, **kwargs):
#         time_steps = x[:, 0].clone().detach()
#         x = self._forward(x[:, 1:], *args, **kwargs)
#         return torch.cat([time_steps, x], dim=1)
...
class Autoencoder(nn.Module):
    def __init__(self, n_features, latent_dim, num_layers, in_seq_length, out_seq_length, recurrent_module=nn.RNN, *args, **kwargs):
        ...
        self.encoder = Encoder(n_features, latent_dim, num_layers, recurrent_module) #nn.Sequential(module(n_features, latent_dim, batch_first=True))
...
class LitTemporalAutoencoder(L.LightningModule):
    def __init__(self, n_features, latent_dim, num_layers, in_seq_length, out_seq_length, recurrent_module, optimizer_class, lr, *args, **kwargs):
        ...
        encoder = Encoder(n_features, latent_dim, num_layers, recurrent_module)
    ...
    def _common_step(self, batch, batch_idx, *args: Any, **kwargs: Any):
        ...
        y_true = x[:, -self.out_seq_length:, :].clone()
        return y_hat, y_true
    ...
    def validation_step(self, batch, batch_idx, *args: Any, **kwargs: Any) -> STEP_OUTPUT:
        y_hat, y_true = self._common_step(batch, batch_idx, *args, **kwargs)
        loss = nn.functional.mse_loss(y_hat, y_true)
        score = -mean_squared_error(y_hat, y_true, squared=False)
        # score = r2_score(y_hat.reshape((batch[0].size(0), -1)), y_true.reshape((batch[0].size(0), -1)))
        self.log_dict({"valid/loss": loss, "valid/score": score, "hp_metric": score}, on_step=False, on_epoch=True)
