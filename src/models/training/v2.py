...
def train_or_load_best(root_dir, model_class: Type[L.LightningModule], model_label: str, autoencoder_version: Optional[str], train_frac: float, hparams_set: Dict[str, Union[str, int, float, bool]],
    ...
    if trainer_kwargs is None:
        trainer_kwargs = {}
    best_model_path = best_model(root_dir, model_label, autoencoder_version, train_frac, hparams_set, ulterior_tags)
    ...
    if best_model_path.exists() and force_retrain:
        shutil.rmtree(model_dir(root_dir, model_label, autoencoder_version, train_frac, hparams_set, ulterior_tags))
    return train_model(root_dir, model_class, model_label, autoencoder_version, train_frac, hparams_set, ulterior_tags, train_ds, val_ds, log, max_epochs, batch_size, trainer_kwargs)
